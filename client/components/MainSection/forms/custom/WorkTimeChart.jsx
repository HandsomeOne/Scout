...
import $ from './WorkTimeChart.css'

const weekdays = ['星期天', '星期一', '星期二', '星期三', '星期四', '星期五', '星期六']

export default class WorkTimeChart extends Component {
  getRanges() {
    ...
    return (
      <Row gutter={16} style={{ marginBottom: 16 }}>
        ...
        <Col span={22} className={$.time}>
          {new Array(7).fill().map((_, i) => <div key={i}>{i * 4 < 10 && '0'}{i * 4}:00</div>)}
        </Col>
      </Row>
    )
  }
...
WorkTimeChart.propTypes = {
  workTime: T.arrayOf(T.arrayOf(T.arrayOf(T.number))).isRequired,
}
